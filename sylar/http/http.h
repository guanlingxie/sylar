...
bool CheckGetAs(MapType &m, const std::string &key, T &val, const T &def = T())
{
    auto it = m.find(key);
    if(it == m.end())
    {
...
template<typename MapType, typename T>
T GetAs(MapType &m, const std::string &key, const T &def = T())
{
    auto it = m.find(key);
    if(it == m.end())
    {
...
public:
    ...
    bool hasHead(const std::string &key, std::string *val = nullptr) const;
