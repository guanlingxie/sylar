    ...
    try {
        for(next = results; next; next = next->ifa_next) {
            ...
            switch(next->ifa_addr->sa_family) {
                ...
                case AF_INET6:
                    {
                        //SYLAR_LOG_DEBUG(g_logger) << "xxxx inet6";
...
IPAddress::ptr Create(const char *address, uint32_t port)
{
    ...
    if(error)
    {
        SYLAR_LOG_ERROR(g_logger) << "IPAddress::Create(" << address << " errno = " << strerror(errno);
        return nullptr;
    }
    try
    {
...
{
    m_addr.sin_port = byteswapNeedTrans(port);
    int result = inet_pton(AF_INET, address, &m_addr.sin_addr.s_addr);
    if(result <= 0)
    {
        SYLAR_LOG_ERROR(g_logger) << "IPv4Address::IPv4Address(" << address << ", " 
            << port << ") rt = " << result << " errnostr = " << strerror(errno);
    }
}
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port)
{
    ...
    m_addr.sin_port = byteswapNeedTrans(port);
    m_addr.sin_addr.s_addr = byteswapNeedTrans(address);
}
...
{
    int32_t addr = byteswapNeedTrans(m_addr.sin_addr.s_addr);
    os  << ((addr >> 24) & 0xff) << "."
        << ((addr >> 16) & 0xff) << "."
        << ((addr >> 8)  & 0xff) << "."
        << (addr         & 0xff)
        << ":" << byteswapNeedTrans(m_addr.sin_port);
    return os;
}
...
IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len)
{
    ...
    baddr.sin_addr.s_addr |= byteswapNeedTrans(~CreateMask<uint32_t>(prefix_len));
...
IPAddress::ptr IPv4Address::subnetMask(uint32_t prefix_len)
{
    ...
    subnet.sin_addr.s_addr = byteswapNeedTrans(CreateMask<uint32_t>(prefix_len));
    return IPv4Address::ptr(new IPv4Address(subnet));
}

uint32_t IPv4Address::getPort() const
{
    return byteswapNeedTrans(m_addr.sin_port);
}

void IPv4Address::setPort(uint16_t v)
{
    m_addr.sin_port = byteswapNeedTrans(v);
}

IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port)
{
    IPv6Address::ptr rt(new IPv6Address);
    rt->m_addr.sin6_port = byteswapNeedTrans(port);
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port)
{
    memset(&m_addr, 0 , sizeof(m_addr));
    m_addr.sin6_family = AF_INET6;
    m_addr.sin6_port = byteswapNeedTrans(port);
    bool flag = (1 == byteswapNeedTrans(1));
    if(flag)
        memcpy(&m_addr.sin6_addr.s6_addr, address, 16);
    else{
        for(int i = 0;i < 16;++i)
            m_addr.sin6_addr.s6_addr[i] = address[15 - i];
    }
}
...
std::ostream &IPv6Address::insert(std::ostream &os) const
{
    ...
    for(size_t i = 0; i < 8; ++i)
    {
        ...
        os << std::hex << (int)byteswapNeedTrans(addr[i]) << std::dec;
    }
    if(!used_zeros && addr[7] == 0)
        os << "::";
    os << "]:" << byteswapNeedTrans(m_addr.sin6_port);
    return os;
}
...
{
    return byteswapNeedTrans(m_addr.sin6_port);
}

void IPv6Address::setPort(uint16_t v)
{
    m_addr.sin6_port = byteswapNeedTrans(v);
}
